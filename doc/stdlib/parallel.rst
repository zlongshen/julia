...
.. data:: AbstractLock
   ...
   Abstract supertype describing types that implement the thread-safe syncronization primitives: ``lock``\ , ``trylock``\ , ``unlock``\ , and ``islocked``
...
.. function:: trylock(the_lock) -> Success (Boolean)
   ...
   Acquires the lock if it is available, returning ``true`` if successful. If the lock is already locked by a different task/thread, return ``false``\ .
...
.. function:: islocked(the_lock) -> Status (Boolean)
   ...
   Check whether the lock is held by any task/thread. This should not be used for syncronization (see instead ``trylock``\ ).
